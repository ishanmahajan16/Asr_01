...
timit_phone_map = phone_maps(mapping_file="D:\deeplearning\Examples\ASR_2018_T01-master\p1\kaldi_60_48_39.map")
...
def _preprocess_data(args):
    ...
    for full_wav in full_wavs:
        ...
        mfcc_features.append(mfcc_feats)
        mfcc_labels.append(labels)
    ...
    try:
        np.save("features_test_delta_delta.npy",mfcc_features)
        np.save("labels_test_delta_delta.npy",mfcc_labels)
    except:
        print("error")
    
    #Possibly separate features phone-wise and dump them? (np.where() could be used)
    
    """path="timit_test_delta.hdf"
    print(path)
    timit_df.to_hdf(path, "timit")"""
